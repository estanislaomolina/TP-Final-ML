...
def calcular_ventanas_temporales(vuelo, df_meteo_horario):
    ...
    # Ventana PRE-vuelo (2 horas antes)
    hora_inicio_pre = max(9, int(hora_despegue) - 2)
    hora_fin_pre = int(hora_despegue)
    datos_pre = dia_data[(dia_data['hora'] >= hora_inicio_pre) & (dia_data['hora'] < hora_fin_pre)]
    
    if len(datos_pre) > 0:
        if 'temp_2m' in datos_pre.columns:
            features_ventana['temp_trend_pre2h'] = datos_pre['temp_2m'].iloc[-1] - datos_pre['temp_2m'].iloc[0]
        if 'cape' in datos_pre.columns:
            features_ventana['cape_mean_pre2h'] = datos_pre['cape'].mean()
            features_ventana['cape_trend_pre2h'] = datos_pre['cape'].iloc[-1] - datos_pre['cape'].iloc[0]
        if 'wind_speed' in datos_pre.columns:
            features_ventana['wind_mean_pre2h'] = datos_pre['wind_speed'].mean()
    ...
    if 'cape' in dia_data.columns and dia_data['cape'].notna().any():
        cape_series = dia_data['cape']
        idx_max_cape = cape_series.idxmax()
        if pd.notna(idx_max_cape):
            hora_cape_max = dia_data.loc[idx_max_cape, 'hora']
            features_ventana['diff_hora_cape_optimo'] = hora_despegue - hora_cape_max
        else:
            features_ventana['diff_hora_cape_optimo'] = np.nan

        cape_max_dia = cape_series.max()
        if pd.notna(cape_max_dia) and cape_max_dia > 0:
            features_ventana['ratio_cape_vs_optimo'] = vuelo.get('meteo_cape', np.nan) / cape_max_dia
        else:
            features_ventana['ratio_cape_vs_optimo'] = np.nan
    else:
        features_ventana['diff_hora_cape_optimo'] = np.nan
        features_ventana['ratio_cape_vs_optimo'] = np.nan
...
def enriquecer_dataset():
    ...
    dataset_file = 'data/processed/dataset_completo_HORARIO.csv'
    ...
    # Guardar
    output_file = 'data/processed/dataset_COMPLETO_ENRIQUECIDO.csv'
